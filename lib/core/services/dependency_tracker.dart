...
class EventDependencyTracker {
  ...
  ///
  /// Checks if [dependentId] can reach [blockerId] through existing
  /// dependencies (which would create a cycle if we add blockerId → dependentId).
  bool wouldCreateCycle(String blockerId, String dependentId) {
    if (blockerId == dependentId) return true;

    // BFS from dependentId's dependents to see if we can reach blockerId
    ...
    // Also check reverse: can we reach dependentId from blockerId through
    // existing blocker chains?
  ...
  ///
  /// Uses Tarjan-like DFS with 3-color marking.
  ...
  Map<String, int> computeDepths() {
    ...
    while (changed && iterations < maxIterations) {
      ...
      for (final id in allIds) {
        ...
        final blockers = getBlockers(id);
        final allBlockersResolved =
            blockers.every((b) => depths.containsKey(b));

        if (allBlockersResolved) {
          final maxBlockerDepth = blockers
              .map((b) => depths[b]!)
              .reduce((a, b) => a > b ? a : b);
  ...
  /// Finds the critical path (longest dependency chain) in the graph.
  ...
  /// Returns events that are ready to work on (all blockers completed or none).
  ...
  /// Returns a topological ordering of events (respecting dependencies).
  ///
  ...
  /// All unique event IDs referenced in any dependency.
